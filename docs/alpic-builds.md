...
- **The runtime most likely carries devDependencies too.** The log shows no second install and no prune, and the runtime's `node_modules` is copied from the stage that installed them. So a server import of a devDependency would not crash on Alpic. Since the console starts from the self-contained bundle it no longer reads `node_modules` at all, and `check:bundle` boots that bundle from an empty directory, so a package the bundle fails to inline is caught on every `make check` rather than hidden by whatever the image happens to carry.
