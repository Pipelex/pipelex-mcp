...
- `pipelex/no-raw-fetch` refuses a bare `fetch(…)` or `globalThis.fetch(…)` outside `src/capabilities/attachment-fetch.ts`, because a raw request to the API would carry the runtime's default `User-Agent` and be counted as anonymous traffic.
