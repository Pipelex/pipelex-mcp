...
**First view**: The MCP host lists the Pipelex tools: `mthds_list_methods`, `mthds_validate`, `mthds_inputs_template`, `mthds_prepare_inputs`, `mthds_run`, `mthds_run_status`, `mthds_run_results`, and `mthds_show_images`, plus `mthds_upload_attachments` on the hosted console only and `mthds_download_artifacts` on the local workshop only (see Deployments). The console also registers `pipelex_request_upload`, which the `run-graph` view calls and which a host honouring the MCP Apps `ui.visibility` key never lists for the model. `mthds_validate` and `mthds_run` carry the same two Skybridge views as before; the others are plain tools whose payloads are small structured data the model reads directly. `mthds_list_methods` is the assistant-first catalog entry point and deliberately adds no third view.
...
The product ships as **two servers from one repo and one capability core**, sharing one logical identity: the same server key (`pipelex`), the same tool names (with the documented per-shell exceptions, below), the same structured contracts, and the same verdict discipline. The capability core (`capabilities/`) knows nothing about which shell invoked it.
...
**Each shell registers its own tool table, and the two differ only by the tools one shell alone can serve.** The workshop's table is `src/local/tools.ts` and the console's is `src/hosted/tools.ts`, both built from the same capability core; a tool on both is defined once per table, and today the two copies read alike. The exceptions — two on the console, three on the workshop — are each deliberate rather than incidental, for the same reason inverted: the tool could never fire on the other shell, so advertising it there would spend every user's tokens on every `tools/list` for a capability that cannot work, and would invite the model to attempt it.
...
- **`pipelex_request_upload` is registered on the hosted console only.** Its one caller is the `run-graph` view's input form (see Run-Form Upload Scope), and the workshop registers no views, so nothing there could call it; a workshop agent uploads a local file through `mthds_prepare_inputs` instead.
...
- **`mthds_save_method` and `mthds_get_method` are registered on the local workshop only** (see Catalog Write Scope). Unlike `mthds_codegen`, whose write argument is optional and which therefore keeps one contract on both shells, the console refusing one field of it, the filesystem is not optional on either side of these two: the save submits the bundle in the `{ path }` form the console rejects and finishes by writing the link file that makes the next save an update rather than a duplicate, and the get exists to bring sources to disk. A console save would mean a materially different act under the same name, which is the invariant below. The console's users reach both gestures in the webapp's editor. Serving their inline halves there later is additive.

`mthds_show_images` is deliberately **not** a third exception: it fetches a run's pictures and answers them as MCP image content blocks, touching no filesystem and needing no host-substituted argument, so it is on both shells and behaves identically on each (see Image Display Scope).

The shells already differ in behavior behind shared names (views on the console only, the `{ path }` resolver and the upload boundary on the workshop only), each set by the shell's own context builder; these exceptions extend that per-deployment split to presence, for the tools whose input or output only one shell can handle. The invariant that holds, and that matters for routing: **one tool name means one contract.** A name both shells register carries the same input schema, output schema and annotations on both, which `src/tool-names.test.ts` enforces; when two shells' contracts for a tool diverge, the tool gets a different name on one of them. The description is outside that rule, because it is how each shell tells its own audience when to call the tool.
...
- **A console-only tool added after the server split was ratified is born `pipelex_*`** — `pipelex_request_upload`. The split renames every console tool with the `pipelex_` prefix in its console release, because the console is the Pipelex account product (its catalog, its storage, its sign-in) and the prefix removes the collision with the workshop's `mthds_*` names in a host that has both. A tool that exists only on the console takes its final name from the start rather than a name it would be renamed away from; the `mthds_*` reasoning above still governs every tool registered before that release.
...
`mthds_prepare_inputs` used to share that expansion and to withhold `method_ref` entirely, both for the same reason: the SDK's `prepareInputs` took inline files only and read the signature from `/v1/build/inputs`, which no shell could reach from an address. Since `@pipelex/sdk` 0.17.0 it takes all three selectors and reads the signature from one `POST /v1/validate` with `views: ["input_form"]`, so **both** shells resolve every selector server-side and the tool is uniform with the rest of the table. The per-shell difference is what it always was — the upload boundary, not the selector set.
...
**Which routes apply the gate, and which do not.** A selector resolved through `POST /v1/validate` or `POST /v1/start` travels `fetched_method_source`, which loads the package's non-`.mthds` files, so the gate applies — `mthds_validate`, `mthds_prepare_inputs` (whose signature is read from `/v1/validate`) and `mthds_run`. `POST /v1/build/inputs` and `POST /v1/codegen` resolve an address through `fetch_method_mthds_files`, which takes only the `.mthds` files and loads no Python, so no execution locus is ever decided and the gate cannot fire — `mthds_inputs_template` and `mthds_codegen` therefore answer an address that the other three refuse. `/v1/start` also applies the gate to a submitted bundle and to a stored method's injected source, which is why the locator follows the request shape rather than always naming `method_ref`.
...
**When the catalog is searched is a per-shell question, and the tool description answers only the part both shells agree on.** The description carries the reactive triggers alone — the user asked what exists, or named a saved method without its id — because the two shells' copies of it read alike and a proactive trigger is right on only one of them. On the hosted console, where the caller has no skills and the catalog is often the whole entry point, discovery is the point, and the console's own `instructions` keep the third trigger: a saved method that may fit the task is a reason to look. The workshop's `instructions` state the reactive triggers and stop there — a coding-agent session is driven by skills, and a proactive clause in text that reaches every session had it searching the catalog in the middle of work no skill had asked it to leave, `/pipelex-design` included. So per-shell guidance lives in each shell's `instructions`, the channel that exists for it, rather than in the description. The tool is untouched by that choice: one name, one contract, one behaviour on either shell.
...
## Catalog Write Scope (`mthds_save_method`, `mthds_get_method`) — local workshop only
...
**Both are registered on the local workshop only, and that is a third per-shell exception rather than a widening of the `output_dir` precedent.** `mthds_codegen` advertises its write argument on both shells because writing is *optional* there: the inline arm is the whole contract and the console refuses one argument instructively. Here the filesystem is not an option on the side of either tool. `mthds_save_method` submits the bundle as `files` in the `{ path }` form the console rejects outright, and it finishes by writing the link file that makes the next save an update rather than a duplicate — a console save would therefore mean a materially different act under the same name, which is the one invariant the per-shell split exists to protect. `mthds_get_method`'s write arm has nowhere to write, and its inline arm exists for the single case of explaining a method to a workshop session that is about to work on it. The console's users reach both gestures in the webapp's editor, which is where the catalog's human surface lives. Serving the inline halves on the console later is additive and needs no contract change here.
...
`include_graph` defaults to true. The graph rides the tool result's view-only `_meta` channel (`_meta.graph_spec`, consumed by the `run-graph` view), never `structuredContent`. When false, omit it entirely. The form's two per-pipe artifacts ride the same channel (`_meta.pipe_io_contracts`, the report's own map keyed by namespaced `pipe_ref`; `_meta.input_form`, the wire input-form descriptor the capability requests via the opt-in `views` token; `_meta.output_form`, its twin for the other half of the contract, requested by the same token list; plus `_meta.main_pipe_ref` — the effective entry pipe, the report's `default_pipe_ref` with the bundle blueprint's `domain.main_pipe` only behind an absent field) on a valid **and runnable** verdict whose two artifacts each carry at least one entry, independently of `include_graph`. Riding and being **advertised** are two different gates, and only the advert speaks for the entry pipe: the pair is view-only data the model never sees, and it is the map the view looks a pipe up in, so gating it on the entry pipe would make a click on a graph node dead for every pipe. The pair is absent on pending-signature verdicts (a form whose Run can only fail is not worth advertising) and when the runner returned no descriptor (the token is lenient, so an older runner just omits `input_form` — the kernel derives the fields from the descriptor, so without it nothing is advertised rather than an empty form).
...
```ts
{
  ...
  main_pipe?: {                                    // present on every valid verdict with an effective entry pipe
...
The graph (`graph_spec`) and the form's artifact pair (`pipe_io_contracts`, `input_form`) are not part of `structuredContent`; on a positive verdict they ride the tool result's view-only `_meta` channel (`_meta.graph_spec`, `_meta.pipe_io_contracts`, `_meta.input_form`, `_meta.output_form`, `_meta.main_pipe_ref`) for the `run-graph` view, so the model never pays their tokens. `_meta.main_pipe_ref` rides every valid verdict with an effective entry pipe, since the signature below needs the same ref — the view ignores it when the artifacts it accompanies are absent. Because the model never sees `_meta`, `available_view_specs` is its signal that a view exists to surface: it lists the renderable view kinds for this result. `"dry_run_graph"` is the method graph from the validation dry run, present exactly when its spec was produced (valid verdict with `include_graph` not false); `"input_form"` is the fill-in form for the entry pipe's inputs, present exactly when both artifacts ride `_meta` **and** the settled entry pipe has its own entry in each (the descriptor being the one the `views` opt-in returns). A form is a form *for* a pipe, so a verdict that settles none advertises none: the model must not be told a form exists for the view to then render one for a pipe nobody chose. The artifacts still ride there, and the view still opens no form of its own — a pipe the user clicks in the graph is the only thing that produces one. The list is empty otherwise. On those same verdicts a short `## Views` note is appended to the `content` summary, worded for whichever views are present, so agents that read the prose more reliably than the structured fields also learn the views exist.
...
- **The entry pipe is the report's `default_pipe_ref`; the blueprint derivation stands only behind an absent field.** `default_pipe_ref` is the API's statement of the pipe a selector-less run of that same request would execute, and it is the only signal that knows a `method_ref` package's manifest (`METHODS.toml`), whose `main_pipe` outranks the bundle-level declaration on the run and build routes. Reading `bundle_blueprint.main_pipe` instead is manifest-blind, so a package where the two differ got a signature typing a call site `mthds_run` would not run. Three arms, and `null` is deliberately not `undefined`: a non-empty string is the stated default; a stated `null` (or any unreadable value) is the server reporting that it determined **none** — no `main_pipe` anywhere, or a manifest naming a pipe the closure does not declare or declares in several domains — which is precisely when a selector-less `mthds_run` would fail to resolve one too, so no signature is emitted and the blueprint is *not* consulted behind it; the field being **absent** means the runner predates it, and only there does the blueprint derivation (its primary blueprint's `main_pipe`, qualified by its domain) stand. A JSON body cannot carry an own property holding `undefined`, so reading the field is the whole absence test.
...
**Both shells advertise `output_dir`, `readOnlyHint: false` and `destructiveHint: true`**, although only the workshop can write. An annotation says what a tool *may* do, and the console refuses the argument instructively (`input_domain` at `output_dir`, naming `npx @pipelex/mcp`) exactly as it refuses `files: { path }`. The alternative — a per-shell contract, the console's copy dropping `output_dir` or its write annotations — was considered and rejected: this repo already ships two precedents for one contract whose affordances one shell refuses, and the invariant that one tool name means one contract is worth more than the cosmetic mismatch.
...
- Exactly one of (non-empty `files`, `method_ref`, `method_id`) is required — the tooling rule from Method Selectors (no selector → `input_domain` at `files`; a second selector, or a blank one → `input_domain` at the offending field). All three are **server pass-throughs** on both shells: the selector rides `POST /v1/validate` (with `views: ["input_form"]`), which resolves an address on the runner and an id on the hosted platform. Nothing is expanded client-side, so this tool no longer carries a `getMethodClosure` leg of its own.
...
- The **hosted console** is **pass-through only**: it accepts `http(s)` URLs and `pipelex-storage://` URIs, and **refuses any input that would require an upload** — a `data:` URL, inline bytes, or a local path — with an instructive `input_domain` no-verdict located at `inputs`, naming the local workshop (`npx @pipelex/mcp`) and the pass-through alternatives. It resolves the signature itself, from the same one `POST /v1/validate` with `views: ["input_form"]` and by the same pipe-selection order, and runs the same descriptor-guided walk with the upload branches replaced by refusals — so the two arms classify file positions identically and differ only in what they do at one. The console never reads a filesystem and never uploads. (Per-user OAuth gives each console caller their own identity, which settles *whose* storage an upload would target — but inline `data:` bytes still bloat the model's context, the exact cost the workshop's `{ path }` design exists to avoid, so console byte upload stays deferred until a proper out-of-band attachment channel exists; see Non-Goals.)
...
> **Enforcement constraint (implementation).** The console must not hand raw `inputs` to the SDK's `prepareInputs`: for a bare-path value at a file position the SDK reads the *server's* filesystem (`readLocalPath`) before failing at the upload boundary — an LFI-read / DoS (`/dev/zero`) / path-existence-oracle hazard on a public endpoint. The read happens *inside* the walk, so no client-side guard on the upload call can forestall it and no value-shape pre-screen can find it without the descriptor. The console therefore resolves the pipe's signature itself and rejects any non-pass-through file-bearing input up front — the console's analogue of the workshop's filesystem resolver, a per-deployment seam rather than a fork of the tool contract. The cost is that the console mirrors the SDK's descriptor walk and its pipe-selection order; a future SDK helper taking a pluggable source resolver would let it reuse the SDK's own walk instead, the same "SDK-canonical, swappable-for-native" posture the by-id expansion had.
...
- `input_domain` — bad request shape (no selector, a second selector, a blank selector, a blank `pipe_ref`); an unqualified or unknown `pipe_ref` and a closure with no single default pipe (both refused client-side before any walk, at `pipe_ref`); an unresolvable closure ("the method signature did not resolve"), which is refused client-side too but locates at **whatever named the method** — `files`, `method_ref` or `method_id` — because the bundle is broken and the caller's `pipe_ref` is not; a selector the route refuses (a `method_ref` that does not parse or fetch at `method_ref`, an unknown or foreign-org `method_id` at `method_id`, a stored method with no MTHDS source at `method_id`, either execution-locus 403 at whatever named the method); a rejected asset (`RejectedAssetError`, a 413 past the service size cap, at `inputs`); an invalid local source (`InvalidLocalSourceError` — missing/unreadable path, at `inputs`); the console's pass-through-only upload refusal (at `inputs`). The two asset arms are workshop-only; the console's refusal is its own.
- `config` — auth failure (`UploadAuthenticationError` / `ClientAuthenticationError`, 401/403, carrying the deployment's auth texture), a paywall (402, the org's plan does not cover the call, tagged `kind: "paywall"`), an unreachable API, a deployment with no upload route (`UnsupportedUploadCapabilityError`, 404), and **a deployment that serves no usable `input_form` descriptor** (at `PIPELEX_BASE_URL`) — no request the caller can write works around it, so reporting it against one of their own fields sent them editing a request that was never the problem.
...
**Three client-side questions, three locators.** They are raised before any request and used to share one, which meant each was reported against whichever field the last one cared about:

- *Which pipe?* — an unqualified or unknown `pipe_ref`, or a closure settling no single default (`ClassifyErrorOptions.preparation`, at `pipe_ref` on all three shapes). Separate from the route's 400/422 because collapsing them told a caller with a perfectly good address and a bad `pipe_ref` that the address was wrong.
- *Which method?* — the closure did not validate (`ClassifyErrorOptions.closure`, at the selector the caller typed). A caller naming a broken published package by `method_ref` was being told to fix a `pipe_ref` they had left empty, on a bundle they do not own.
- *Which deployment?* — no usable descriptor came back (`ClassifyErrorOptions.missingDescriptor`, `config` at `PIPELEX_BASE_URL`). Its message named the deployment while its hint named the pipe, so the two contradicted each other in the same error.

The deployment auth texture is threaded from the context's `authError` as before.
...
4. The view sends the file straight to the app bucket with `@pipelex/sdk/upload`'s `uploadWithGrant`, bounded by its own timeout (a minute plus a second per 128 KiB, since the SDK sets none), and hands the reference to the panel only once storage has answered.
...
**App-only.** The tool carries `_meta.ui.visibility: ["app"]`, the MCP Apps standard's key for a tool the view calls and the model does not, which OpenAI's Apps SDK reference names as the preferred form over its own `openai/visibility`. Its description is written for a host that ignores the key: it says the view calls the tool itself and sends a chat attachment to `mthds_upload_attachments`.
...
// structuredContent
{
  ...
  image_candidates?: Array<{ uri: string; key: string }>;  // state=completed only, and only when the output references stored files — the free image shortlist, see below
...
## Image Display Scope (`mthds_show_images`) — both shells

`mthds_show_images` puts the pictures a completed run produced in front of the model, as MCP **image content blocks**. It is the deliberate counterpart to `mthds_run_results`' free inventory: the results tool says what is there and fetches nothing; this one fetches and shows. It is a plain tool — no Skybridge view, no `_meta` channel, no `available_view_specs`.
...
**Registered on both shells.** Unlike `mthds_download_artifacts`, nothing here touches a filesystem: the tool reads a run and answers content blocks, which every host can carry. A call nobody can make by accident is safe everywhere, and having the same tool on the console is what lets a later view button be a view change rather than a capability change.
...
// structuredContent
{
  status: "ok" | "error";
  run_id?: string;
  state?: "running" | "completed" | "failed";   // the same reading mthds_run_results reports
  retry_after_seconds?: number | null;          // state=running only
...
**The summary** names how many pictures follow and that they are now part of the conversation, lists any withheld candidate under `## Withheld` with its reason or its error (the `mthds_codegen` pattern), and, on a shell that registers the download tool, reminds the caller the full files can be saved. A completed run with no candidate at all is one sentence and no blocks.

**Behaviour is identical on the two shells**, which is itself the point: the structured contract, the caps and the blocks are the same, and the only difference is the one prose sentence that names `mthds_download_artifacts` where that tool exists.
...
**Write policy.** Containment is the shared write-side boundary described in Deployments, and it stays this tool's own: the directory is real-path-checked on its deepest existing ancestor before it is created and again after, before the SDK writes a byte into it. The *filename* and the *collision rule* are the SDK's (`artifactFilename` and its exclusive create), and both follow from where the names come from. The filename is never taken from anything the caller typed: it is the last segment of the storage key, reduced to `[A-Za-z0-9._-]` with leading dots stripped (no hidden file, no `..`), capped in length with the extension preserved where one fits, and given an extension from the object's content type when the key carries none (a short table: the image, PDF, text and JSON types a run produces; an unknown type gets no extension rather than a guessed one). An empty result falls back to `artifact-N`. Path separators are the split point, so no traversal survives. **Files are never overwritten**: the file is created exclusively (`wx`) and a name collision gets a numeric suffix (`name-1.ext`). Since a few files download at once, which of two same-named files takes the suffix is not fixed. A collision here means two *different* files, so suffixing is the right answer — the exact inverse of `mthds_codegen`'s write arm, whose paths come from the engine and whose regeneration must land on the same names. The two writers share containment and nothing above it.
...
```ts
{
  ...
  artifacts?: Array<{                           // state=completed only — one per storage reference, in discovery order; its length is the count walked
...
1. When the user asks what saved methods exist or names one without an id — and, on the hosted console only, when a saved method may plausibly solve the task — the assistant calls `mthds_list_methods` (with a focused `query` when possible). Which of those triggers each shell states is Catalog Discovery Scope above.
...
2. The assistant calls `mthds_download_artifacts` with the run id (optionally a `dir` relative to the working directory). The tool walks the run's full output, resolves the references to fresh links through the API, and saves the files under the working directory — never overwriting, suffixing on collision.
...
**Server instructions**: The server sets a short MCP `instructions` string (server-wide, on the `McpServer` constructor options) that hosts surface to the model. It is the map, not the manual, and it is front-loaded, because a host that cuts keeps the head: it opens with the order of the steps (`mthds_list_methods` to find a saved method, then validate, fill the inputs template, prepare, run, status and results, and show or download what the run produced), then names `mthds_codegen` (and, on the workshop, the catalog pair `mthds_save_method` / `mthds_get_method`), states the three ways to name a method **once** for every method-taking tool, and ends with the rules that hold everywhere: `mthds_run` spends inference credit, and a picture `mthds_show_images` returns stays in the conversation for every turn that follows, so it is shown when someone wants to look at it rather than by reflex. `mthds_list_methods` is prompted when the user asks what saved methods exist or names one without its id — and, on the hosted console only, when a saved method may fit the task, per Catalog Discovery Scope. The instructions do not embed a dynamic catalog because hosted auth is request-scoped and host instructions may be cached. Each shell's instructions additionally say what only that shell does: the workshop's prefer the `{ path }` file form, name `mthds_download_artifacts`, and say there are no views; the console's name `mthds_upload_attachments` for a file the user attached in the chat, say that `mthds_prepare_inputs` there only passes references through, and say that a valid verdict shows the user an interactive graph. **The selector sentence names every method-taking tool and both selectors**, because a sentence that named `method_id` and omitted `method_ref` told the model a published method could not be validated, templated, prepared or run — so the by-address flow was never offered, however fully the tools supported it; a unit test holds that over the five selector-taking tools on both shells, and a second holds that the step order is complete, in order, within the first few hundred characters. Everything per-tool stays out: when to call a tool, and the contract rules needed before calling it (which forms may be combined, what an absent `method_id` means to a save), live in the tool `description`; parameter detail, such as the address grammar, lives in the field descriptions.

**Text budget**: Each shell's `instructions` and every tool `description` are held to 1,800 Unicode code points, measured on what the servers emit by `npm run check:tool-texts`, which `make check` and CI run. Claude Code cuts each of those texts at 2,048 characters, so a text over the cap reaches the model with its tail missing; the ceiling leaves room for one added sentence and a margin for hosts whose cap has not been measured. A fact goes in the last layer that still reaches the model when it needs it: the instructions carry the map (what the server is for, the three ways to name a method, the order of the steps, the rules that apply everywhere), a tool description says when to call that tool and states the contract rules needed before calling it, a field description carries parameter detail, and a result summary carries what matters only after the call.
...
- **Annotations**: Read-only, non-destructive, no open-world publishing.
- **View**: `run-graph` — renders `_meta.graph_spec` with `@pipelex/mthds-ui`'s `GraphViewer` (inline preview plus a user-triggered fullscreen toggle) and, on a runnable verdict with a settled entry pipe, that pipe's input form below it with `RunPanel` over `_meta.input_form` and `_meta.pipe_io_contracts` (Run starts `mthds_run` from the view with the validation's own selector and follows it; a file-bearing input takes a picked file through `pipelex_request_upload` — see Run-Form Upload Scope); compact empty state when there is neither. The form is for `_meta.main_pipe_ref` or the pipe node the user clicked, never a pipe of the view's own choosing — with no entry pipe no form opens on its own, though a clicked pipe still gets one. When the graph's own pipe (`graph_spec.pipeline_ref`, the bundle's declared main pipe) is not the entry pipe, a caption under the graph names both. Works identically whichever selector supplied the validated content.
...
**Tool: `pipelex_request_upload`** — hosted console only, app-only (see Deployments and Run-Form Upload Scope)
...
- **Output**: `{ status, uri?, expires_at?, max_bytes?, errors? }` in `structuredContent`, plus a short text summary naming the reference and pointing a model at `mthds_upload_attachments`. The grant itself rides `_meta.upload_grant` for the view alone.
...
- **Behavior**: One-shot result lookup (`GET /v1/runs/{id}/results`), discriminated on `state`: `running` (with retry hint), `completed` (bounded `main_stuff` + `truncated` flag), `failed` (terminal status + failure message; no graph exists for failed runs). On `completed` it also reports the free stored-file inventory — `image_candidates` and one merged prose sentence naming `mthds_show_images` and, on the workshop, `mthds_download_artifacts` — walking the full output in memory and **fetching nothing**.
- **Annotations**: Read-only, non-destructive, no open-world publishing.
...
- **Registered on both shells**, unlike the two per-shell tools: nothing here touches a filesystem or needs a host-substituted argument.
...
- **Output**: `{ status, run_id?, state?, retry_after_seconds?, run_status?, failure_message?, scope?, artifacts?, saved_paths?, all_saved?, errors? }` in `structuredContent`, plus a text summary that repeats the saved paths (relative to the working directory, which it names once) and any per-file failure. No `_meta`, no `available_view_specs`.
- **Behavior**: Reads the run (`GET /v1/runs/{id}/results`) and branches on its state like `mthds_run_results`; on `completed`, walks the full main output for `pipelex-storage://` references and hands the results to the SDK's `downloadArtifacts`, which resolves them to fresh presigned links in one call to `POST /v1/resolve-storage-url/bulk` and streams the bytes into files under the working directory within the SDK's download boundary (http(s) only with plain http under the base-URL rule, no redirects, bounded timeout, per-file and per-call byte caps, no partial file left behind). Filenames are sanitized from the storage key and never overwrite an existing file; the directory is real-path-contained on both sides of `mkdir` before the SDK writes into it. Partial success is a produced verdict.
