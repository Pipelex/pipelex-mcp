...
Every method-taking tool (`mthds_validate`, `mthds_inputs_template`, `mthds_prepare_inputs`, `mthds_run`) selects the method it operates on in one of three forms — the platform's addressing contract, stated once here so the per-tool sections carry only their own mechanics:
