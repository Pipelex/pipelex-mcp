...
- **`structuredContent.image_candidates`**, on both shells: one `{ uri, key }` entry per stored reference whose storage key passes an extension prefilter (`.png`, `.jpg`, `.jpeg`, `.gif`, `.webp`, or no extension at all — the runtime does store extensionless keys, and only a fetched content type can settle those). The member is **absent**, not empty, when the output references no stored file, so a consumer can tell "nothing was produced" from "nothing looked like an image". The walk is `collectArtifacts` plus a string test: no link is resolved, no byte is read, and nothing reaches the model but a handful of short strings. It is a **shortlist, never a verdict** — the content type at fetch time is what decides, which is `mthds_show_images`' job.
...
// structuredContent
{
  ...
  failure_message?: string;                     // state=failed only
  images?: Array<{                              // state=completed only — one entry per candidate CONSIDERED, in discovery order
    uri: string;
  ...
  all_inlined?: boolean;                        // state=completed only; vacuously true for an empty walk
  errors?: ToolError[];
}
```
