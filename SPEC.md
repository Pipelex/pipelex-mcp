...
- **The output is rendered by the form kernel.** It is `@pipelex/mthds-ui/form/react`'s `StuffViewer` in the kernel's `app` presentation, the result renderer `pipelex-app` and the method apps use, with its Rendered / JSON switch and its Download control, over the field `buildResultField` derives from the executed pipe's output descriptor (`_meta.output_form`) and its contract's payload schema (`_meta.pipe_io_contracts`, the output's `json_schema`). It is fed the full output, `_meta.main_stuff`, never the bounded copy the model reads, so nothing is cut for the user. When the result cannot describe its output — no descriptor, no payload schema, or no way to tell which pipe ran — the panel shows the payload through the kernel's `JsonView` and says why.
