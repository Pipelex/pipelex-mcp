...
4. Once the run is terminal, the assistant calls the results tool and reports the main output (bounded) on success, or the failure message otherwise. On the console the view's completion handoff prompts it to (see the console's Views).
...
- **Annotations**: Read-only, non-destructive, no open-world publishing.
- **View**: `run-graph` — the method's graph and, on a runnable method, the input form whose Run button calls `pipelex_run`.
...
- **View**: `run-follow` — the self-polling live status card; on the terminal outcome it fires the once-per-run completion handoff.
...
4. The start returns the durable `run_id`, and the `run-follow` card follows the run live. When the run is terminal, the completion handoff prompts the assistant, which reports through `pipelex_run_results`, and `pipelex_show_images` shows a picture the run produced when someone asks to see it.
