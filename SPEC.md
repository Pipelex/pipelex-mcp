...
- `config` — upload auth failure (`UploadAuthenticationError` / `ClientAuthenticationError`, 401/403, carrying the console's reconnect-and-sign-in texture), a paywall (402), an unreachable API, a deployment with no upload route (`UnsupportedUploadCapabilityError`, 404).
