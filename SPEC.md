...
1. **Tooling tools (`mthds_validate`, `mthds_inputs_template`, `mthds_codegen`, `mthds_prepare_inputs`, `pipelex_show_method`): exactly one selector.** Give files, an address, or an id — never several, and on the console one of the two references. A second selector is an instructive `input_domain` no-verdict located at the extra field (mirroring the API's own 422). These operations are stateless: there is no Run row, so "linkage" has no referent, and an extra selector could only be ignored — the worst contract of the three. This retires the old "files win, `method_id` is ignored" behavior, which was exactly the folklore the rule replaces.
...
- Exactly one of `method_id` / `method_ref`, by the rule in Method Selectors: no selector is `input_domain` at `method_id`, a second one is `input_domain` at the extra field, and a blank one is `input_domain` at itself. A blank `pipe_ref` is `input_domain` at `pipe_ref`.
...
**It reads `POST /v1/validate`**, with signatures allowed and the views tokens `["input_form", "output_form"]`, the selector riding the request body as a server pass-through, and no rendered Markdown, since the tool composes its own summary. That route already serves everything the tool needs for an id and an address alike — the graph, the IO contracts, both form descriptors and `default_pipe_ref` — and it reports a method whose dry run fails, or whose signatures are pending, as not runnable instead of handing out a form whose Run can only fail. The cost is the dry run's CPU and latency on every show, with no inference. The move to `POST /v1/input-form`, which serves the descriptor without a dry run, will change the route and not this contract, so it will ask no ChatGPT user to re-add the connector. This is not a validate tool: its verdict answers only whether the method can run.
...
- **`pipe_ref`, not `pipe_code`.** The tool maps it onto the run route's `pipe_code`. Only a pipe the caller named rides the start: the input walk picks the same default the run route does, and naming the walk's choice on the start would let a disagreement between the two run a pipe nobody chose.
