...
- `input_domain` — bad request shape (no selector, a second selector, a blank selector, a blank `pipe_ref`); an unresolvable closure (`InputPreparationError` "the method signature did not resolve"), an unqualified or unknown `pipe_ref`, and a closure with no single default pipe (all three refused client-side by the SDK before any walk, at `pipe_ref`); a selector the route refuses (a `method_ref` that does not parse or fetch at `method_ref`, an unknown or foreign-org `method_id` at `method_id`, a stored method with no MTHDS source at `method_id`); a rejected asset (`RejectedAssetError`, a 413 past the service size cap, at `inputs`); an invalid local source (`InvalidLocalSourceError` — missing/unreadable path, at `inputs`); the console's pass-through-only upload refusal (at `inputs`). The two asset arms are workshop-only; the console's refusal is its own.
...
Each is classified in `classifyError` (extended for the `InputPreparationError` family — these derive from `PipelineRequestError`, so they must be mapped ahead of the generic arm) with its `retryable` verdict, using per-route `ClassifyErrorOptions` picked by the request's selector shape, exactly as `mthds_validate` and `mthds_codegen` do: a route 400/422 locates at `files`/`pipe_ref`, `method_ref` or `method_id` according to what the caller supplied. **A client-side signature failure is a separate question and gets a separate locator** (`ClassifyErrorOptions.preparation`): the SDK raises it before any request, and it is always about the pipe whatever named the method, so it locates at `pipe_ref` on all three shapes. Collapsing the two told a caller with a perfectly good address and a bad `pipe_ref` that the address was wrong. The deployment auth texture is threaded from the context's `authError` as before.
