...
- **Annotations**: Read-only, non-destructive, no open-world publishing.
- **View**: `run-graph` — renders `_meta.graph_spec` with `@pipelex/mthds-ui`'s `GraphViewer` (inline preview plus a user-triggered fullscreen toggle) and, on a runnable verdict, the main pipe's input form below it with `RunPanel` over `_meta.input_form` and `_meta.pipe_io_contracts` (Run starts `mthds_run` from the view and follows it; see UI Overview for the spike's boundaries); compact empty state when there is neither. Works identically whether the validated content came from `files` or a `method_id` fetch.
