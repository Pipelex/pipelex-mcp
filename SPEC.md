...
```ts
{
  ...
  inputs: Record<string, unknown>; // the caller's FILLED compact inputs (the mthds_inputs_template output, populated)
}
```
...
- `inputs` is **required** (it is the whole point — the filled values to prepare). An empty object `{}` is accepted (nothing to prepare); it passes through and uploads nothing.
