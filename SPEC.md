...
**The tool table is shared except for the tools only one shell can serve.** Both shells register the same tool table from `src/tools.ts`, with documented exceptions — one on the console, three on the workshop — and each is deliberate rather than incidental, for the same reason inverted: the tool could never fire on the other shell, so advertising it there would spend every user's tokens on every `tools/list` for a capability that cannot work, and would invite the model to attempt it.
...
**When the catalog is searched is a per-shell question, and the shared tool description answers only the part both shells agree on.** The description carries the reactive triggers alone — the user asked what exists, or named a saved method without its id — because one description serves both shells and a proactive trigger is right on only one of them. On the hosted console, where the caller has no skills and the catalog is often the whole entry point, discovery is the point, and the console's own `instructions` keep the third trigger: a saved method that may fit the task is a reason to look. The workshop's `instructions` state the reactive triggers and stop there — a coding-agent session is driven by skills, and a proactive clause in text that reaches every session had it searching the catalog in the middle of work no skill had asked it to leave, `/pipelex-design` included. So per-shell guidance lives in each shell's `instructions`, the channel that exists for it, rather than in a description that would have to say the same thing to both. The tool is untouched by the split: one name, one contract, one behaviour on either shell.
