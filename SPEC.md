...
- `pipe_ref` is the pipe to project, as a qualified `domain.pipe_code`. Optional; it defaults server-side to the closure's declared `main_pipe` (for a `method_ref`, the manifest's `main_pipe`), which fails as a no-verdict error (API 422) when the closure declares none or several across its domains.
...
// input
{
  ...
  pipe_code?: string;                // pipe to run; omitted → server resolves the bundle's (or the manifest's) main pipe
