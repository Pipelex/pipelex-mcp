...
**It registers no views.** None of its results carries the graph or the form's artifacts on `_meta`, and `available_view_specs` is always empty on every tool that declares it. `mthds_validate` still accepts `include_graph`, which changes nothing here. The text summaries carry the whole flow.
...
`include_graph` defaults to true and has no effect on the workshop, which registers no views: the graph and the form's artifacts are view-only data, and they ride `_meta` only on a server whose context says it renders views — on the console that is `pipelex_show_method`, which runs this same projection (see Show Method Scope for how those artifacts ride and are advertised).
