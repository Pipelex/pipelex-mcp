...
**Each shell registers its own tool table, and the two differ only by the tools one shell alone can serve.** The workshop's table is `src/local/tools.ts` and the console's is `src/hosted/tools.ts`, both built from the same capability core; a tool on both is defined once per table, and today the two copies read alike. The exceptions — one on the console, three on the workshop — are each deliberate rather than incidental, for the same reason inverted: the tool could never fire on the other shell, so advertising it there would spend every user's tokens on every `tools/list` for a capability that cannot work, and would invite the model to attempt it.
