...
// structuredContent
{
  ...
  omitted?: number;                             // state=completed only, and only when something was left out
...
**The walk carries its own deadline, because a per-image timeout is per image.** The walk is sequential, so six stalled objects accumulated their timeouts into a call of three minutes and more — each fetch also pays a resolve round-trip before its own timer starts — and a host whose deadline is shorter then failed the *whole* call, losing the pictures that had already arrived along with the reasons for the ones that had not. So `INLINE_IMAGES_DEADLINE_MS` (60s) bounds the walk end to end: each fetch is given whichever is smaller, its own timeout or the time left, and a candidate reached with none left is `withheld: "deadline"` without being attempted. Partial results always come back.
