...
**Output file trust model**: an image or a document in the output renders in the results panel only inside two containment layers. First, the kernel paints only a URL a browser can load — `http(s)`, and also `data:`, `blob:` and a root-relative path, which resolves against the view's own origin; a `pipelex-storage://` reference resolves nowhere in a browser, and the panel supplies no resolver — preferring the payload's presigned `public_url`, and it previews a document in a frame only when a browser renders it with nothing installed (a PDF or an image). Second, and this is the layer that holds, each view's CSP names exactly the hosted platform's per-env storage buckets, where run outputs are served as presigned URLs, never a wildcard: `resourceDomains` for an image, `frameDomains` for a document's preview frame. The runtime signs those links path-style on the regional endpoint (`https://s3.us-west-2.amazonaws.com/pipelex-app-{dev,staging,prod}/…`, measured on 2026-09-25), a host every bucket in the region shares, so the sources name each bucket as a path, which CSP matches as a prefix, beside the regional virtual-hosted origins (`pipelex-app-{dev,staging,prod}.s3.us-west-2.amazonaws.com`) a runtime naming an addressing style would produce. Any other host or bucket, a third-party generation-provider URL leaking through `main_stuff` included, is refused by the host's CSP before a request leaves the browser, and the kernel names the file instead. These URLs stay view-only: they ride `_meta` and are never surfaced into model context (consistent with the `result_url` rule above). **The presigned links expire**: the runtime bakes a `public_url` into the stored result when the run writes it, and the results route relays it as written, so a conversation reopened after the link's life shows a broken image where the picture was. Painting a fresh link on reopen needs a resolver the view can call, which is a tool the console does not have yet.
