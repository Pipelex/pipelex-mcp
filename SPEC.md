...
The product ships as **two servers from one repo and one capability core**, and the two share no tool name. The capability core (`src/capabilities/`) holds the API calls, the projections, error classification, the run lifecycle and the image walk. It knows nothing about which server invoked it beyond the settings a server states on the contexts it builds, such as whether it renders views or which tool names its texts use. Each server owns its tool table, its schemas, its descriptions and its instructions: `src/hosted/tools.ts` for the console and `src/local/tools.ts` for the workshop.
...
**One tool name means one contract.** A name both servers registered would have to carry the same input schema, output schema and annotations on both, which `src/tool-names.test.ts` enforces; when two servers' contracts for a tool diverge, the tool gets a different name on one of them. Since the console's tools became `pipelex_*`, no name is shared, and the test is what keeps a future shared name honest. The description is outside that rule, because it is how each server tells its own audience when to call the tool.
...
**The template is projected client-side**, never fetched from a build route: `src/capabilities/inputs-template.ts` runs the MTHDS standard's own projection (`mthds/protocol`'s `projectInputsTemplate`) over the pipe's `input_form` descriptor, in the explicit `{ concept, content }` shape, as a JSON object. It is present only on a runnable method with a settled pipe. A descriptor the projection cannot walk yields no template rather than a failure, and the summary then tells the model to build the inputs from the signature, one entry per input name. A file input takes an `http(s)` URL or a `pipelex-storage://` reference in its `url`.
...
- **The schema is declared in the capability layer, not imported from `skybridge/server`.** Skybridge ships an equivalent `FileRef`, but the capability core is Skybridge-free by construction (`src/capabilities/`, `src/tool-definition.ts` and the workshop's `src/local/` import zero Skybridge symbols) and importing it would drag Skybridge into the tsup-bundled workshop binary. A local Zod object emits a byte-identical JSON Schema, so the constraint costs nothing.
...
The console carries no inputs template tool: `pipelex_show_method` projects its template client-side from the input-form descriptor (`src/capabilities/inputs-template.ts`, over `mthds/protocol`'s projection), in the same explicit JSON shape this tool returns by default.
