...
```ts
{
  ...
  main_pipe?: {                                    // present on every valid verdict declaring a main pipe
...
The graph (`graph_spec`) and the form's artifact pair (`pipe_io_contracts`, `input_form`) are not part of `structuredContent`; on a positive verdict they ride the tool result's view-only `_meta` channel (`_meta.graph_spec`, `_meta.pipe_io_contracts`, `_meta.input_form`, `_meta.main_pipe_ref`) for the `run-graph` view, so the model never pays their tokens. `_meta.main_pipe_ref` rides every valid verdict that declares a main pipe, since the signature below needs the same ref — the view ignores it when the artifacts it accompanies are absent. Because the model never sees `_meta`, `available_view_specs` is its signal that a view exists to surface: it lists the renderable view kinds for this result. `"dry_run_graph"` is the method graph from the validation dry run, present exactly when its spec was produced (valid verdict with `include_graph` not false); `"input_form"` is the fill-in form for the main pipe's inputs, present exactly when both artifacts ride `_meta` (valid, runnable, non-empty contracts, and the descriptor returned by the `views` opt-in). The list is empty otherwise. On those same verdicts a short `## Views` note is appended to the `content` summary, worded for whichever views are present, so agents that read the prose more reliably than the structured fields also learn the views exist.
...
- **Source of truth is `pipe_io_contracts[main_pipe_ref]`**, a required field on every valid report and not gated by the `views` token. The input-form descriptor is consulted for exactly one thing, **input order** — the contract's `inputs` map deliberately contracts none, the descriptor states authored order — and when the descriptor (or its entry) is absent the map's own order stands. The signature never depends on the descriptor being present.
- **Present on every valid verdict, pending signatures included, independent of `viewsAvailable` and `include_graph`.** The workshop has no views and is precisely the deployment the integrating agent uses, so this must not ride the views branch the way the `_meta` artifacts do; a pending-signature verdict has a fully determined signature, and knowing the shape before the signatures resolve is useful. Absent when the blueprint declares no `main_pipe`.
- **Known limit — by-address entry selection.** The ref is derived from the report's `bundle_blueprint` (its primary blueprint's declared `main_pipe`). For a `method_ref` package the run and build routes default their pipe to the **manifest's** `main_pipe` first (`METHODS.toml`), which can differ from — or exist without — the bundle-level declaration; such a package gets a signature for a pipe `mthds_run` will not run by default, or no signature at all. The validate response does not carry the manifest's entry today, so this cannot be resolved client-side; it is tracked as cross-repo work against the Pipelex API (expose the effective entry pipe on validate-by-address), after which this capability prefers that field.
...
- **One rendered signature line is appended to the `content` summary** on the same verdicts, ahead of the `## Views` note, in MTHDS-flavoured notation — `demo.main(document: legal.Contract, notes?: native.Text[]) -> analysis.Report[]`, where `?` marks an input the caller may omit (and an output a successful run may resolve absent), `[]` a variable list and `[N]` a fixed one. Agents read prose more reliably than structured fields, and the summary is the one channel that reaches a ChatGPT install whose cached tool list predates the schema change.
