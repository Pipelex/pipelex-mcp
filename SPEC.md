...
- **Defensive narrowing, whole-signature omission.** Every member is checked against what actually arrived (strings non-empty, multiplicity in the closed vocabulary, `item_count` a number exactly on the fixed arm, presence in the marker vocabulary). Any malformed member, or no contract entry for the declared main pipe, omits the **whole** signature rather than emitting a partial one — half a signature actively misleads the agent typing against it, where an absent one just sends it to `mthds_inputs_template`. The verdict is never affected.
