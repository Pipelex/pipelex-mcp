...
**The workshop shell imports no Skybridge at all.** `src/local/` builds a plain MCP-SDK stdio server; the only Skybridge symbol anywhere under it is a type-only `OAuthConfig` import in `src/local/server.test.ts`. So the two live detectors this repo is proud of — `make smoke`, which drives the workshop over stdio, and `make test-e2e`, which reaches the real API through the real `PipelexApiClient` — run the whole capability core **without ever loading the console shell**. They will pass with a completely broken console. Do not let a green e2e run stand in for a check on this bump.
