...
**The workshop shell imports no Skybridge at all.** `packages/workshop` builds a plain MCP-SDK stdio server, and neither it nor the core it inlines imports or declares anything from Skybridge; the OAuth stand-in the console's shell tests use lives in the console, in `packages/console/src/hosted/test-oauth.ts`. So the two live detectors this repo is proud of — `make smoke`, which drives the workshop over stdio, and `make test-e2e`, which reaches the real API through the real `PipelexApiClient` — run the whole capability core **without ever loading the console shell**. They will pass with a completely broken console. Do not let a green e2e run stand in for a check on this bump.

What `make all` genuinely covers is narrow but real: `skybridge build` has to succeed (it regenerates the gitignored view-name registry `packages/console/.skybridge/views.d.ts`, which is why `build` runs before `typecheck`), and `tsc` then checks that the console's `src/server.ts`, `src/hosted/server.ts`, `src/helpers.ts` and views still typecheck against the new `.d.ts` files. A renamed export or a changed signature does fail there, loudly. That is worth having.

What it cannot see is everything that only exists at run time or at deploy time:

- whether the server boots against the **real** WorkOS AuthKit (the OAuth provider is resolved with a top-level `await` in `packages/console/src/server.ts`, and `workosProvider` performs discovery over the network);
...
**One gate does start the built console: `check:bundle`**, the step of `make check` right after the build. The console is started from `packages/console/dist/server.bundle.js`, which `packages/console/scripts/emit-server-bundle.mjs` copies out of the Vercel build output `skybridge build` writes (`.vercel/output/functions/mcp.func/index.js`, under the console), and `packages/console/scripts/check-server-bundle.mjs` boots that copy from an empty directory, with no `node_modules` anywhere above it, against a local stand-in for AuthKit. It proves the bundle is self-contained, that it boots, that it refuses an anonymous call, and that an authenticated client gets the `tools/list` and view resources `packages/console/src/hosted/console.contract.json` pins. So a release that moves the Vercel output, leaves a new package external, or changes what registration emits fails `make check`. It does not prove the four items above.
...
for p in skybridge @skybridge/devtools; do
  echo "$p"
  echo "  range:      $(node -p "const m=require('./packages/console/package.json'); m.dependencies?.['$p'] ?? m.devDependencies?.['$p']")"
...
**`skybridge` sits in the console's `dependencies` and `@skybridge/devtools` in its `devDependencies`, and neither belongs in any other manifest.** The console's server imports `skybridge`, so its own manifest declares it; `@skybridge/devtools` serves only `make dev`. The console is never published, so neither reaches an `npx @pipelex/mcp` install from there, and at run time the console reads neither from `node_modules`, since it starts from the server bundle, which inlines everything `skybridge/server` reaches. What would hurt is either package appearing in the workshop's or the core's manifest, which would put Skybridge's non-optional peers (`react`, `react-dom`, `vite`, `nodemon`, `@skybridge/devtools`) back into every workshop install; `tests/workspace-manifests.test.ts` fails when `skybridge` is declared anywhere but the console. Read the console's `package.json` diff in Step 11 all the same.

A dirty tree is not a blocker, but if `packages/console/package.json`, `package-lock.json` or `packages/console/CHANGELOG.md` is already dirty, say so and agree how to keep your edits separable before you start.
...
**The peer pin is a range, not a version — it constrains the devtools target, it does not name it.** `skybridge@1.4.1` pins `@skybridge/devtools: ^1.1.0`, so "what the pin says" is a floor that `1.1.0` and `1.4.1` both satisfy; substituting it into Step 5 would write `^1.1.0` into the console's `package.json` and silently lower the declared floor. So resolve a **concrete** `<devtools-target>`: take the newest published `@skybridge/devtools` that satisfies the pin and does not cross past `<target>`'s major — in practice the same version number as `<target>`, since the two ship together — and say out loud which version you settled on. If the newest devtools does **not** satisfy the pin, stop and show the user: the lockstep assumption has broken and a version has to be chosen by hand.

Diff those two peer sets and show the user. A peer that **appeared** may force another bump in this repo (a new `vite` floor, say); a peer that **vanished** is usually harmless but tells you the framework stopped owning something. Check each surviving peer against what this repo declares, and **work from the diff you just computed rather than from any list written here** — the peer set is version-specific and a major rewrites it. `react`, `react-dom` and `vite` are declared by the console and have been peers throughout; `nodemon` is a peer this repo leaves to npm's automatic peer install. The two that move are the lesson: at `1.4.1` the peers include `@modelcontextprotocol/sdk >=1.27.0` and **not** `zod`, while at `2.0.0` that inverts — `zod ^4.2.0` is added as a peer of `skybridge` itself and `@modelcontextprotocol/sdk` drops out of the peer set into Skybridge's own dependencies. This repo declares its own floor for both (`@modelcontextprotocol/sdk ^1.29.0`, `zod ^4.3.6`, in the `dependencies` of the members that import them, at one range across the workspace), so either can be the one that forces a second bump depending on which version you are moving to. None of the peers is optional — neither release declares `peerDependenciesMeta` — so every one of them binds.
...
Four wiring points, none of them in the console's `src/`, and each fails in its own quiet way:

- **`packages/console/vite.config.ts`** imports `skybridge()` from `skybridge/vite`. A plugin option change surfaces at build time, so `make all` covers it.
- **`packages/console/.skybridge/views.d.ts`** is gitignored and regenerated as `skybridge build`'s first step. No Make target removes it — `make clean` takes each package's `dist`, `coverage` and the `*.tsbuildinfo` files only — but `skybridge build` rewrites it, so `make all` refreshes it in passing. `rm -rf packages/console/.skybridge packages/console/dist` is how to force it by hand.
- **`packages/console/nodemon.json`** overrides Skybridge's default dev exec with `tsx --env-file-if-exists=../../.env src/server.ts`, and watches the core's source beside the console's. Two hazards: a `nodemon.json` **replaces** Skybridge's watch defaults rather than extending them, so a change to what Skybridge watches never reaches us; and if a major splits the entrypoint (app definition in one file, `run()` in another), this `exec` must be repointed at whichever file actually runs.
- **The Makefile's `dev` and `dev-tunnel` recipes append `--port "$CONSOLE_PORT"`** to pin the console to 6843, because the WorkOS Resource Indicator names the port and Skybridge's default walks up from 3000 when it is busy. `tests/make-dev-recipe.test.ts` asserts the recipe *text* and executes only its guard prelude under `sh` — it never invokes the Skybridge CLI. So a renamed or removed `--port` flag leaves that test green and breaks `make dev` for everyone.

### 4d — The deploy surface

Alpic's `startCommand` in the root `alpic.json` runs `node packages/console/dist/server.bundle.js`, and the `Dockerfile` copies the console's `dist` into its runtime image and runs `node dist/server.bundle.js`: both start a copy of the console's `.vercel/output/functions/mcp.func/index.js`. That path is Skybridge's Vercel build output, not a promised interface, and it is where a release is most likely to break this repo without touching a line of source. The bundle is not optional in this workspace: on Alpic the core's `node_modules` entry is a link to a directory the runtime image does not carry. Three things guard it, and each tells you something different when it fails:

- `packages/console/scripts/emit-server-bundle.mjs` (the last step of the console's `npm run build`) refuses when the file is missing, when it is older than this build's `dist/__entry.js`, or when the function directory holds a file other than the bundle, `.vc-config.json` and `package.json`. The first two mean Skybridge moved or stopped writing its Vercel output; find where the bundle went before anything else. The third means esbuild now ships a file beside the bundle (a native addon, say), which the copy would leave behind.
...
There is no Makefile switch for these two (the `use-npm-*` targets are `@pipelex`-only). **Edit both version strings in `packages/console/package.json` first, then resolve once from the root** — each in the block it already sits in:

```jsonc
// packages/console/package.json — two edits, in place
...
**Two edits then one resolve, rather than two `npm install` commands — and that ordering is the point.** The packages are peer-coupled: `skybridge@2.0.0` peers `@skybridge/devtools@^2.0.0`, non-optionally. So `npm install --save-prod skybridge@2.0.0` run on its own, while the old `@skybridge/devtools@^1.4.1` is still declared beside it, asks npm to place a package whose peer contradicts a root dependency — which is the canonical `ERESOLVE` shape, and it aborts before the second command ever runs. Editing both entries first means npm sees one consistent manifest and resolves it in a single pass. It also makes the block question moot: nothing is being re-added, so nothing can infer the wrong block.
...
Show the user the `packages/console/package.json` diff, check the two entries did not swap blocks, and check that no other manifest moved.
...
1. **It boots.** A throw here is the console's `src/server.ts` refusing (read the message — it names the fix) or `workosProvider` failing discovery against the new version.
...
## Step 10 — Update the console's `CHANGELOG.md`

A Skybridge bump reaches the console alone, so its entry goes in `packages/console/CHANGELOG.md`, never in the workshop's. Entries accumulate under `## [Unreleased]` — create that heading above the newest version heading if a release has consumed it. This repo's format is `## [x.y.z]` with **no `v` prefix**.
...
1. Stage **only** what this bump touched — `packages/console/package.json`, `package-lock.json`, `packages/console/CHANGELOG.md`, plus whatever Steps 4 and 9 migrated. Never `git add .` or `git add -A`.
2. Read the `packages/console/package.json` diff one more time for the block boundary: `skybridge` under `dependencies`, `@skybridge/devtools` under `devDependencies`, and neither in another manifest.
...
Then *offer* — do not run — a `/rev` pass, pushing, and opening a PR. PRs target `dev`; only a release branch (`release/vX.Y.Z` or `release/console-vX.Y.Z`) may target `main` here. A major migration deserves the review pass before the PR opens.
...
- Keep `skybridge` and `@skybridge/devtools` in the console's manifest alone; `tests/workspace-manifests.test.ts` fails when `skybridge` is declared anywhere else.
