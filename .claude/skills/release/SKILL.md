...
On the push the run checks out its own merge commit, and its `detect` job settles first whether that commit is a release at all: it reads `package.json`'s version there and at the commit's first parent, and a push is a release exactly when it brings `main` a version `main` did not have. A push that brings none ends green in seconds, with the gate, both surfaces and the tag all skipped. When it is a release, `guard` refuses to go on unless `CHANGELOG.md` carries a `## [X.Y.Z]` heading for that version; it then re-runs `make all` on the merge commit rather than trusting the pull request's own run, and asks `npm view` whether the version is already published, failing the run outright when the registry cannot answer rather than guessing at it. That lookup decides the npm leg and nothing else — the console deploy and the tag no longer read it, so a release whose version reached npm by another route still ships the console and still gets tagged. It is skipped outright on a `-f target=alpic` dispatch, so an unreachable registry cannot fail the guard and take down the very door you reach for when npm is what went wrong. A dispatch answers to one check more, that the typed `version` equals what the commit carries, and it reads `main` whatever ref it was fired from, so it can neither ship a branch nor ship the wrong bump. From there:

- **The `@pipelex/mcp` package on npm** — the local workshop shell, whose `files` list publishes `dist/local`, `README.md` and `LICENSE`, with `prepack` rebuilding `dist/local` so the tarball is built from the commit being shipped. It goes out as `npm publish --access public --provenance` under npm trusted publishing, so no npm token exists anywhere; **the registration is bound to the filename `release.yml`**, and renaming or moving that workflow breaks publishing until the trusted publisher is re-registered. The `guard` job asks `npm view` about the package `package.json` names, at `X.Y.Z`, and the publish step is skipped when the version is already there, so neither a re-run of the merge's run nor a dispatch at the same version can publish twice. The name is read from the manifest rather than typed into the workflow, so the check cannot come to ask about something other than what is published.
...
- **The `vX.Y.Z` tag** — the `tag` job, which needs `detect`, `publish` and `deploy` and runs only when the run is shipping both surfaces, which a merge always is and a one-leg dispatch never is. It creates the tag with `git tag`, so the tags here are **lightweight**, and it leaves an existing tag alone.
...
The console leg is verified from that run's *Deploy the hosted console to Alpic* job, which writes what it shipped into the run summary; the endpoint the README publishes is <https://mcp.pipelex.com/mcp>. A partial failure is retried one leg at a time — `-f target=npm` or `-f target=alpic`, same version — and never by bumping the version. A release finished that way carries no tag, because the `tag` job runs only for a both-surface run: dispatch `-f target=both` at the same version to reach it, which is safe by the workflow's own guards, since the publish step skips a version already on npm, an Alpic deploy is idempotent per commit, and the tag step leaves an existing tag alone. Re-running the merge's own run reaches the tag too, and finishes the release whatever npm already holds, because what the run reads is the commit and not the registry. The two doors ship from different commits, and that is the thing to know: a re-run is pinned to the merge commit that triggered it, so it ships the release however `main` has moved since, while a dispatch reads `main`'s tip and refuses outright once something has landed there, since a tip that does not raise the version is not a release commit. There is one thing a re-run will not do, and it is the reason the door is safe to leave open: a commit carrying a version `main` has already moved past is refused, so re-running an *older* release's run cannot roll the Production console back to that older tree, nor point npm's `latest` dist-tag at the older package. That refusal lives in each shipping leg rather than only in `detect`, which is what makes it hold for the retry you are most likely to reach for: GitHub's "Re-run failed jobs" re-runs only the jobs that failed and the jobs below them, so `detect`'s verdict arrives as an output written before the newer release existed, and a leg that trusted it would ship the downgrade green. A release `main` has moved past is finished by cutting a new version, never by reaching back for its run. Every job of a run then checks out one SHA, resolved once by `detect`, so the tree that ships, the provenance npm records and the tag all name the same commit.
