...
`pipelex-mcp` exposes MTHDS validation, inputs projection, and durable method runs to MCP hosts. It ships as **two servers from one capability core** (see the Architecture section): a hosted **Skybridge console** for remote-connector hosts (ChatGPT, claude.ai, Cowork) and a local **workshop** stdio server distributed on npm as `@pipelex/mcp` for coding-agent hosts (Claude Code, Codex, Cursor). Both register the same MCP tools — `mthds_validate`, `mthds_inputs_template`, and the `mthds_run` / `mthds_run_status` / `mthds_run_results` durable-run family. An assistant that already holds `.mthds` file contents (or, on the workshop, their on-disk paths) calls `mthds_validate` for a stable, structured verdict it can use to explain and repair diagnostics, `mthds_inputs_template` for a fill-in template of a pipe's declared inputs, and the run family to execute a method durably. On a positive validation verdict the `mthds_validate` tool ships a Skybridge view, `run-graph`, that renders the method graph interactively with `@pipelex/mthds-ui`'s `GraphViewer` (hosted console only — the workshop is tools-first, no views); `mthds_inputs_template` and the status/results tools are plain tools with no view.
...
- `capabilities/inputs.ts` — the inputs-template logic. Zod input/output schemas (`files?` beside `method_id?`, `pipe_ref?`, `explicit?` default true, `format?` default json), the API call (`buildInputs`, adapting the MCP `uri` provenance label to the build envelope's `source`), and projection of the report into `structuredContent` plus a composed Markdown summary that includes the template in a fenced code block (the build routes return a plain `message`, not `rendered_markdown`). A `method_id`-only request **fetches-and-forwards** via the shared `fetchMethodFiles` (in `capabilities/shared.ts`, also used by `mthds_validate`): `getMethod` → `methodSourceToContents` → files, each labeled with the method id as provenance, adapted to the build envelope's `source` here; a source-less stored method is an `input_domain` no-verdict at `method_id` raised without calling the build route, and fetch-leg failures classify against `/v1/methods/{id}`. Files win when both are supplied (no linkage concept on the build routes). No view, no `_meta`.
- `capabilities/run.ts` — the durable-run family (`mthds_run` / `mthds_run_status` / `mthds_run_results`) over the SDK's run lifecycle (`start` / `getRunStatus` / `getRunResult`). Starts from `files?` and/or `method_id?`: the platform resolves `method_id` natively on `/v1/start` (id-only runs the stored method's current content; beside files it rides `extra.method_id` as run-history linkage while the files run). Also owns `main_stuff` bounding (the prune ladder under `MAIN_STUFF_CAP`) and the per-route classify options, including the by-id variants picked by request shape.
- `capabilities/method-source.ts` — `methodSourceToContents`, the parser for the polymorphic stored `MethodData.mthds` (raw source / JSON file-array / empty → no source), mirroring the platform's canonical implementation in pipelex-platform's `execution.py`.
...
Every files-taking tool (`mthds_validate`, `mthds_inputs_template`, `mthds_run`) shares one per-item input union: inline contents (`{ content, uri? }`) or a file path (`{ path }`). The arms are non-strict with first-match semantics — a pathological item carrying both keys parses as the content arm and `path` is ignored (strict arms were rejected in Phase 0: an unknown-key failure would surface as a noisy zod union error from the SDK layer instead of our instructive `ToolError`).

On all three files-taking tools (`mthds_validate`, `mthds_run`, `mthds_inputs_template`), `files` is **optional** beside a separate top-level `method_id?` (a registered method's catalog id — deliberately not a third union arm: a method id is not a file, and a mixed array would falsely suggest merging). At least one of the two is required (`validateFilesOrMethodIdRequest`); inline files always win. On `mthds_validate` and `mthds_inputs_template` (routes with no by-id support) a `method_id`-only request resolves via **fetch-and-forward** (`fetchMethodFiles`); on `mthds_run`, `method_id` resolves natively server-side on `POST /v1/start` — see Run Scope.
