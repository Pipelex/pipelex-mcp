 ...
 * copied into dist/ by scripts/emit-server-bundle.mjs, so that nothing is read from
 * node_modules at run time: Alpic's runtime image carries the root node_modules and the
 * console's build output and nothing else, so the workspace's symlink to the core,
 * `@pipelex/mcp-core`, dangles there. That bundle is Skybridge's Vercel output, not a
 * promised interface, and two ways it can go wrong are invisible to every other gate: a
 * package left external that only an installed node_modules satisfies, and a server that
 * bundles but no longer boots. Alpic would refuse such a deployment at its validation
 * step, which is the wrong place to learn it.
 *
