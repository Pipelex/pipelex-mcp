...
//
// Why a bundle at all: the console is a member of an npm workspace, and its
// server imports the core, `@pipelex/mcp-core`, a workspace package that is
// never published. Starting from `dist/__entry.js` means resolving that import
// from `node_modules` at run time, through the workspace's symlink, and Alpic's
// runtime image holds only the root `node_modules` (copied from its install
// stage, before the build) and the console's build output: the symlink points
// at a directory the image never received, and a trial deploy that started
// from `dist/__entry.js` failed at boot with ERR_MODULE_NOT_FOUND (see
// docs/alpic-builds.md). The bundle inlines every package the server reaches,
// the core and `skybridge` included, so nothing is read from `node_modules`
// once it runs.
//
...
// every `make check`. Both paths are relative to the console's package.
...
if (!existsSync(SOURCE)) {
  fail(
    `${path.relative(ROOT, SOURCE)} does not exist. \`skybridge build\` used to emit its Vercel ` +
      ...
      "because the console starts from dist/server.bundle.js, and the core it inlines reaches no node_modules of the runtime image.",
  );
}
