 ...
 *   REGIONAL endpoint, so both views load a run's images, and frame its
 *   documents, from `<bucket>.s3.us-west-2.amazonaws.com`;
 ...
 *
 * So `run-graph`, which also sends a picked file with a grant, may connect to both forms:
...
/** Where the runtime's presigned read links point: where both views load a run's images and frame its documents from. */
