...
import { consoleHost, userAgentOf } from "@pipelex/mcp-core/capabilities/client-identification.js";
import type { RequestHeaders } from "@pipelex/mcp-core/capabilities/client-identification.js";
...
/**
 * Sourced from the console's own package.json, like the workshop's
 * `LOCAL_SERVER_INFO`, so the handshake reports the console's shipped release;
 * the two servers are released on separate tracks. The console is the Pipelex
 * MCP, so it is named `pipelex`; the workshop, the Pipelex plugin's server, is
 * `pipelex-plugin`.
 */
