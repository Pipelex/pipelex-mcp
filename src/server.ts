...
import type { MthdsFile, PipelexValidationResult } from "mthds";
import { z } from "zod";
...
const server = new McpServer(
  ...
  async ({
    ...
    const inputErrors = validateRequest(files, bundle_uri);
    ...
    try {
      const client = new MthdsApiClient({
        baseUrl: process.env.MTHDS_API_URL ?? DEFAULT_LOCAL_API_URL,
      });
      const report = await client.validateFiles(toMthdsFiles(files), {
        allowSignatures: allow_signatures ?? false,
        render: render_markdown ? ["markdown"] : undefined,
      });

      const envelope = validationEnvelope(report, include_graph !== false);
      return toolResult(envelope);
    } catch (err) {
      return toolResult(errorEnvelope("Validation did not produce a verdict.", [
        classifyError(err),
      ]));
    }
  },
...
function toolResult(envelope: ValidationEnvelope) {
  return {
