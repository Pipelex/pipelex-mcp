...
/** The shipped package version, read from `package.json` so it cannot drift. */
 ...
 * the platform attributes the call to the right surface; it has no comment
 * because there is no shell and no host to report.
 */
export const BARE_APP_INFO: AppInfo = { name: MCP_TOKEN_NAME, version: MCP_VERSION };
 ...
 * The `host=` parameter is omitted when there is no host, and also when it would
 * push the header past its ceiling — the SDK refuses an over-long `appInfo` at
 * construction, and a host name must never be able to fail a tool call.
 */
export function mcpAppInfo(
  ...
  const bare: AppInfo = { ...BARE_APP_INFO, details: [mode] };
  if (host === undefined) return bare;
  const withHost: AppInfo = { ...BARE_APP_INFO, details: [mode, `host=${host}`] };
