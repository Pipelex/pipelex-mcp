...
**Phase 3 in progress — paused mid-phase (2026-07-15) at a clean, all-green state.** `make check` + `make t` green (97 tests); everything below is committed on `feature/Run`. What is DONE: (1) the §7-Q2 spike **passed on DevTools** — view-initiated `useCallTool` polling of `mthds_run_status` works with no confirmation, and `mthds_run_results`'s `_meta` passes through as `CallToolResponse.meta` (`graph_spec` + `main_stuff` both arrived); per-host matrix + a spike implementation gotcha (pin `callToolAsync` in a ref — its identity changes per render and a deps-listed poll effect cancels itself) recorded in the design doc §7-Q2; ChatGPT/Claude arms are PENDING and need Louis (Alpic tunnel + connector). (2) The polling core (`src/views/run-polling.ts`: `nextPollDelayMs` ladder 2s→10s + server-hint clamp 1s..30s; `isTransientPollError`: runtime + config@PIPELEX_BASE_URL transient, auth/input_domain hard) with colocated Node tests. (3) The `useRunPolling` + `useElapsedSeconds` hooks (`src/views/use-run-polling.ts`: timers, visibilitychange pause with immediate read on return, stop-on-terminal/hard-error). (4) The full `run-follow.tsx` view (status card with friendly labels/elapsed/spinner/health notes, terminal results fetch incl. mid-write-race retry, GraphViewer + output preview with image-URL narrowing, failure/hard-error/start-error cards, fullscreen toggle, `data-llm` + `useViewState` mirror). (5) Server wiring: `view: { component: "run-follow" }` on `mthds_run`, `startResult` now emits `available_view_specs: ["live_run_status"]` + a `## Views` note (test updated). What REMAINS in Phase 3 (see unchecked boxes): end-to-end DevTools verification of the REAL card on a live run (the spike verified the plumbing, not the shipped card — incl. reopen/remount re-resolve), the ChatGPT/Claude host spike arms, the optional §6.6 button (decide cheap-or-drop), and Checkpoint 3. NOT yet committed as a checkpoint — no `/code-review` fan-out has run on this work yet.
...
- [ ] Optional, keep only if cheap (§6.6): user-triggered "Summarize in chat" button on the terminal card via `sendFollowUpMessage` (canned "The run completed — report the results" prompt). Droppable without touching contracts. (Not built yet — decide at verify time.)
- [ ] Verify the REAL card in DevTools (the spike verified plumbing with a throwaway view, before the shipped card existed): watch a live hosted run start→terminal without any model turn (status card → completed card with graph + output preview); reopen/re-execute and confirm the view re-resolves by id; check the dark theme + fullscreen toggle render. Then **at least one real host** (ChatGPT or Claude, with Louis): same flow + confirm the assistant can answer "is it done?" from the view state without a tool call — this also closes the §7-Q2 ChatGPT/Claude matrix arms in the design doc.
