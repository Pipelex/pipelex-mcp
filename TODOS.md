...
- [x] Completed results: executed `graph_spec` and the **full** (unbounded) `main_stuff` ride `_meta` (keys mirror the API field names, per the existing `graph_spec` convention); bounded copy + fenced ```json summary block in the model-facing streams (the `mthds_inputs_template` duplication pattern).
...
- (P2) `ClassifyErrorOptions` gained a per-route `serverError` hint override, driven by the Q3 live check: the hosted `/v1/start` answers 503 for an invalid bundle, so the start route's 5xx hint points at `mthds_validate`/`mthds_inputs_template` before blaming the platform.
